...
void initGameWindow( GameWindow *gameWindow ) {
    if ( !gameWindow->initialized ) {
        ...
        if ( gameWindow->initAudio ) InitAudioDevice();
        SetTargetFPS( gameWindow->targetFPS );    
        if ( gameWindow->loadResources ) loadResourcesResourceManager();

        gameWindow->gw = createGameWorld();

        // game loop limpo: Apenas atualiza e desenha o mundo
        ...
        if ( gameWindow->loadResources ) unloadResourcesResourceManager();
